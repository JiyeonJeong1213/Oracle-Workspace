/*
    ...
    INLINKE-VIEW : FROM절에 서브쿼리를 사용한 개념(다중열, 다중행 서브쿼리)
...
-- VIEW 특징
-- 1. 컬럼에 대해 산술연산 및 함수의 실행결과를 포함할 수 있다. 단, 반드시 별칭을 부여해야 함
...
-- DECODE함수 이용해서 사원의 이름, 사번, 직업명, 남자인지 여자인지 성별 정보, 근속년수를 VIEW로 저장
...
/*
    ...
    만약에 내가 업데이트 하고자 하는 컬럼이 가상컬럼이라면 DML이 불가능 함
*/
...
SELECT * FROM JOB; -- 실제 JOB테이블에 8번째 행 추가됨
...
/*
    DML문으로 조작이 불가능한 경우
    1. 뷰에서 정의하고 있지 않은 컬럼을 조작하는 경우
    2. 뷰에 포함되지 않은 컬럼 중 베이스가 되는 컬럼에 NOT NULL제약조건이 설정되어 있을 때
...
-- 3. WITH CHECK 옵션 : SELECT문의 WHERE절에서 사용한 컬럼을 "수정하지 못하게" 하는 옵션
...
UPDATE V_CHECKOPTION 
SET DEPT_CODE = 'D7'
WHERE EMP_ID = 207; -- 업데이트 실패(WITH CHECK OPTION)

ROLLBACK;

-- 4. WITH READ ONLY 옵션 : VIEW 자체를 수정하지 못하게 차단하는 옵션(대부분 이렇게 함)
...
UPDATE V_READONLY SET EMP_NAME = '민경민'; -- 업데이트 실패(WITH READ ONLY)