...
WHERE REF_BNO = 22 AND R.STATUS = 'Y'
...
/*
    ...
    - START WITH : 상위타입(최상위 부모)으로 사용할 행을 지정(서브쿼리로도 작성 가능)
    - CONNECT BY : 상위타입과 하위타입의 관계를 규정
                => PRIOR(이전)연산자와 같이 사용하여 현재 행 이전에 상위타입이 있을지 하위타입이 있을지 규정
    ...
    - ORDER SIBLINGS BY : 계층 구조를 정렬시켜주는 구문
    ...
    ** 계층형 쿼리가 적용된 SELECT문 해석 순서 **
    ...
    3 : CONNECT BY
    6 : ORDER SIBLINGS BY
    - WHERE절이 계층형 쿼리보다 해석순서가 늦기 때문에 먼저 조건을 반영하고 싶은 경우 FROM절에 서브쿼리를 이용할 것
*/
