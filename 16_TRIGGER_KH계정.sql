/*
    ...
    EX) 
    - 회원 탈퇴시 기존의 회원테이블에 데이터 DELETE 후 곧바로 탈퇴된 회원들만 따로 보관하는 테이블에 자동으로 INSERT 처리해야 함
    - 신고 횟수가 일정 수를 넘었을 떄 묵시적으로 해당 회원을 블랙리스트 처리되게끔
    - 입출고에 대한 데이터가 기록될 때마다(INSERT될 때마다) 해당 상품에 대한 재고수량을 매번 수정(UPDATE)해야 될 때 등
    ...
    * 트리거 종류
    ...
    > ROW TRIGGER(행트리거) : 해당 SQL문이 실행될 때마다 매번 트리거 실행(FOR EACH ROW옵션 기술해야 됨)
...
-- TB_PRODUCT테이블에서 200번 상품 10개를 증가시키는 UPDATE문 실행 (옛날방식)
UPDATE TB_PRODUCT
  SET STOCK = STOCK+10
WHERE PCODE = 200;
...
/*
    ...
    - 상품이 출고된 경우 -> 해당 상품을 찾아서 재고수량을 감소 UPDATE
*/
...
BEGIN
    ...
    IF (:NEW.STATUS = '입고')
        THEN
        UPDATE TB_PRODUCT
        SET STOCK = STOCK + :NEW.AMOUNT -- :NEW변수 활용해서 TB_PRODETAIL에 인서트된 값을 활용할 수 있음
...
/*
    트리거 장점
    1. 데이터 추가, 수정, 삭제시 자동으로 데이터관리를 해줌으로써 데이터 무결성 보장
    2. 데이터베이스 관리의 자동화
    
    트리거 단점
    1. 빈번한 추가, 수정, 삭제시 ROW의 삽입, 추가, 삭제가 함께 실행되므로 성능상 좋지 못함
    2. 관리적 측면에서 형상관리가 불가능하기 때문에 관리에 불편함
    3. 트리거를 남용하게 되면 예상치 못한 사태가 발생할 수 있으며 유지보수가 힘듦
*/
