/*
    ...
    -> 순차적으로 겹치지 않는 번호를 채번해야 할 때 사용할 예정
    ...
    [
        ...
        *캐시메모리란? 
        ...
        매번 호출할 때마다 새로이 번호를 생성하는 것보다 캐시메모리 공간에 미리 생성된 값들을 가져다 쓰게 되면 속도가 더 빠름
...
/*
    ...
    단, 시퀀스 생성 후 첫 NEXTVAL은 START WITH로 지정된 시작값으로 발생 따라서 시퀀스 생성 후 첫 CURRVAL은 수행이 불가능함 (무조건 NEXTVAL호출 후 CURRVAL호출해야 함)
*/
SELECT SEQ_EMPNO.CURRVAL FROM DUAL;
-- 시퀀스가 생성되고 나서 NEXTVAL을 한번이라도 수행하지 않는 이상 CURRVAL을 수행할 수 없다.
--> CURRVAL은 마지막으로 성공적으로 발생된 NEXTVAL의 값을 저장해서 보여주는 임시값이기 때문
...
/*
    ...
    ALTER SEQUENCE 시퀀스명
    -- START WITH 시작숫자 => 처음 발생시킬 시작값 (DEFAULT 1)
    -- START WITH는 변경이 불가능함 : STARTWITH를 바꾸고 싶다면 시퀀스를 삭제했다가 재생성 해줘야 함
    INCREMENT BY 증가값 => 한번 시퀀스 증가할 때마다 몇씩 증가할지 결정 (DEFAULT 1)
    MAXVALUE 최대값 => 최대값
    MINVALUE 최소값 => 최소값
    CYCLE/NOCYCLE => 값의 순환 여부
...
START WITH 300
MAXVALUE 400;
-- 사번이 추가될 때 실행할 INSERT문 작성 사번 컬럼에 시퀀스를 이용하여 데이터 추가
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, SAL_LEVEL, HIRE_DATE)
VALUES (SEQ_EID.NEXTVAL, '홍길동', '123456-1234567', 'J1', 'S1', SYSDATE);
