/*
    ...
    필요할 때마다 내가 작성한 PL/SQL문을 편하게 호출 가능함
...
SELECT * FROM RPO_TEST;

-- 프로시져 생성
CREATE PROCEDURE DEL_DATE
IS
BEGIN 
    DELETE FROM RPO_TEST;
    COMMIT;
END;
/
...
/*
    ...
    EX) DB에서 대용량의 데이터를 SELECT문으로 받아와서 자바에서 처리하는 경우 VS
        DB에서 대용량의 데이터를 SELECT 한 후에 자바로 넘기지 않고 직접 처리하는 경우
        DB에서 처리하는 게 당연히 성능이 더 좋음 (데이터를 넘길 때마다 네트워크자원을 소비하게 됨)
        
    프로시져 단점
    1. DB자원을 직접 사용하기 때문에 DB에 부하를 주게 됨(남용하면X)
    2. 관리적 측면에서 자바소스코드, 오라클 코드 동시에 형상관리하기 어려움
    
    정리)
    한번에 처리되는 데이터량이 많고 성능을 요구하는 처리는 대체로 자바보다는 DB상에서 처리하는 것이
    성능측면에서는 나을 것이고, 소스관리(유지보수) 즉 형상관리 측면에서는 자바가 더 좋다.
*/
--------------------------------------------------------------------------------------
/*
    <FUNCTION>
    프로시져와 거의 유사하지만 실행결과를 반환 받을 수 있음
...
RETURN VARCHAR2
IS
    RESULT VARCHAR2(1000); -- 변수선언 여기서 함 DECLARE X
BEGIN
